...
// Ported from romp-hive. Pointy-top axial coordinates. Slot i counts a center-out spiral:
// slot 0 is the origin and ring k holds 6k slots, so the board grows evenly in every
// direction and a slot index alone fixes a pad's place. Sessions KEEP their slot for life
// (assignSlots) — the board never reshuffles on a push, only on a real arrival or departure.
...
// rather than islands. The pads' slight base flare (hive.ts ×1.04) tucks under the shared rim.
export const HEX_SIZE = 2.05;
export const PAD_R = HEX_SIZE;

// Prism orientation — the turn that makes flush pads FIT instead of overlap. Three's
...
// exact angles axialToXZ hands the six neighbours. Flat hexes (RingGeometry/CircleGeometry)
// walk θ as x = r·cosθ, y = r·sinθ and lie down via rotation.x = −π/2, landing a vertex at
// XZ angle −θ: −π/6 lines their corners up with the prism's.
...
// target. A huge step (tab restored after minutes) caps at 50ms so nothing teleports.
